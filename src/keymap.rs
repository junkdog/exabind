...
impl KeyMap {
    ...
    pub fn actions_by_category(&self, category: &str) -> &[Action] {
        self.actions.get(category).unwrap_or(&EMPTY_ACTIONS)
    }
...
impl IntoKeyMap for PathBuf {
    fn into_keymap<F: FnOnce(&str) -> KeyMap>(self, f: F) -> KeyMap {
        ...
        let mut file = std::fs::File::open(self).expect("file to be present");
        file.read_to_string(&mut input).expect("parsable xml");
